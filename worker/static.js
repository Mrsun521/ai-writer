// 前端静态页面嵌入 — 由 Worker 直接 serve，无需 GitHub Pages

export const STATIC_FILES = {
  "/": `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="description" content="AI 写作助手 - 一键生成周报、小红书文案、朋友圈文案，智能 AI 写作工具">
  <meta property="og:title" content="AI 写作助手 - 一键生成优质内容">
  <meta property="og:description" content="周报 · 小红书 · 朋友圈，一键生成">
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>✍️</text></svg>">
  <title>AI 写作助手 - 一键生成优质内容</title>
  <link rel="stylesheet" href="/css/style.css">
</head>
<body>
  <nav class="navbar">
    <div class="navbar-inner">
      <a href="/" class="navbar-logo">AI 写作助手</a>
      <ul class="navbar-links">
        <li><a href="/">首页</a></li>
        <li><a href="/tools/weekly">工具</a></li>
        <li><a href="/console">控制台</a></li>
        <li><a href="/pricing">价格</a></li>
      </ul>
      <div class="navbar-usage">
        <span class="usage-badge">剩余 10 次</span>
      </div>
    </div>
  </nav>
  <main class="main-content">
    <section class="hero">
      <div class="container">
        <h1>AI 写作助手</h1>
        <p>周报 · 小红书 · 朋友圈，一键生成</p>
        <a href="/tools/weekly" class="btn btn-primary btn-lg">立即使用</a>
        <p class="hero-scroll">开始创作 ↓</p>
      </div>
    </section>
    <section class="container">
      <div class="card-grid">
        <div class="card">
          <div class="card-icon">📋</div>
          <h3>周报生成器</h3>
          <p>输入本周工作内容，自动生成专业周报，支持亮点总结和下周规划。</p>
          <div class="card-usage">免费次数：<span class="free"><span class="free-count">10</span> 次</span></div>
          <a href="/tools/weekly" class="btn btn-primary btn-block">开始使用</a>
        </div>
        <div class="card">
          <div class="card-icon">📕</div>
          <h3>小红书文案</h3>
          <p>智能生成小红书爆款文案，标题吸睛、正文动人、标签精准。</p>
          <div class="card-usage">免费次数：<span class="free"><span class="free-count">10</span> 次</span></div>
          <a href="/tools/xiaohongshu" class="btn btn-primary btn-block">开始使用</a>
        </div>
        <div class="card">
          <div class="card-icon">💬</div>
          <h3>朋友圈文案</h3>
          <p>根据场景和心情生成朋友圈文案，工作生活节日全覆盖。</p>
          <div class="card-usage">免费次数：<span class="free"><span class="free-count">10</span> 次</span></div>
          <a href="/tools/moments" class="btn btn-primary btn-block">开始使用</a>
        </div>
      </div>
    </section>
  </main>
  <footer class="footer">
    <div class="container">
      <p>&copy; 2026 AI 写作助手. All rights reserved.</p>
    </div>
  </footer>
  <script src="/js/api.js"></script>
  <script src="/js/app.js"></script>
</body>
</html>`,

  "/pricing": `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="description" content="AI 写作助手 - 一键生成周报、小红书文案、朋友圈文案，智能 AI 写作工具">
  <meta property="og:title" content="AI 写作助手 - 一键生成优质内容">
  <meta property="og:description" content="周报 · 小红书 · 朋友圈，一键生成">
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>✍️</text></svg>">
  <title>价格 - AI 写作助手</title>
  <link rel="stylesheet" href="/css/style.css">
</head>
<body>
  <nav class="navbar">
    <div class="navbar-inner">
      <a href="/" class="navbar-logo">AI 写作助手</a>
      <ul class="navbar-links">
        <li><a href="/">首页</a></li>
        <li><a href="/tools/weekly">工具</a></li>
        <li><a href="/console">控制台</a></li>
        <li><a href="/pricing">价格</a></li>
      </ul>
      <div class="navbar-usage">
        <span class="usage-badge">剩余 10 次</span>
      </div>
    </div>
  </nav>
  <main class="main-content">
    <div class="container">
      <section class="hero" style="padding-bottom:20px">
        <h1>选择适合你的方案</h1>
        <p>按需付费，灵活使用</p>
      </section>
      <div class="pricing-grid">
        <div class="pricing-card">
          <span class="badge popular">免费</span>
          <div class="plan-name">体验版</div>
          <div class="plan-price">免费</div>
          <div class="plan-desc">体验 AI 写作的魅力</div>
          <ul class="plan-features">
            <li>10 次免费生成</li>
            <li>所有工具可用</li>
            <li>基础文案生成</li>
          </ul>
          <a href="/tools/weekly" class="btn btn-primary btn-block">开始使用</a>
        </div>
        <div class="pricing-card featured">
          <span class="badge popular">按次</span>
          <div class="plan-name">按次付费</div>
          <div class="plan-price">1 <span class="unit">元/次</span></div>
          <div class="plan-desc">用多少付多少，灵活自由</div>
          <ul class="plan-features">
            <li>1 元/次，无上限</li>
            <li>所有工具可用</li>
            <li>优先生成速度</li>
            <li>微信支付</li>
          </ul>
          <div style="display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin:16px 0">
            <div>
              <img src="data:image/jpeg;base64," alt="微信收款码" style="width:140px;height:auto;aspect-ratio:1/1;object-fit:contain;border-radius:8px">
              <p style="font-size:12px;color:var(--text-muted);margin-top:4px">微信支付</p>
            </div>
            <div>
              <img src="data:image/jpeg;base64," alt="支付宝收款码" style="width:140px;height:auto;aspect-ratio:1/1;object-fit:contain;border-radius:8px">
              <p style="font-size:12px;color:var(--text-muted);margin-top:4px">支付宝</p>
            </div>
          </div>
          <p style="font-size:13px;color:var(--text-muted)">扫码支付，即时到账</p>
        </div>
        <div class="pricing-card">
          <span class="badge coming">即将上线</span>
          <div class="plan-name">包月套餐</div>
          <div class="plan-price">19.9 <span class="unit">元/月</span></div>
          <div class="plan-desc">高频使用首选，超值优惠</div>
          <ul class="plan-features">
            <li>100 次/月生成额度</li>
            <li>所有工具可用</li>
            <li>优先生成速度</li>
            <li>专属客服支持</li>
          </ul>
          <button class="btn btn-secondary btn-block" disabled>即将上线</button>
        </div>
      </div>
    </div>
  </main>
  <footer class="footer">
    <div class="container">
      <p>&copy; 2026 AI 写作助手. All rights reserved.</p>
    </div>
  </footer>
  <script src="/js/api.js"></script>
  <script src="/js/app.js"></script>
</body>
</html>`,

  "/tools/weekly": `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="description" content="AI 写作助手 - 一键生成周报、小红书文案、朋友圈文案，智能 AI 写作工具">
  <meta property="og:title" content="AI 写作助手 - 一键生成优质内容">
  <meta property="og:description" content="周报 · 小红书 · 朋友圈，一键生成">
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>✍️</text></svg>">
  <title>周报生成器 - AI 写作助手</title>
  <link rel="stylesheet" href="/css/style.css">
</head>
<body>
  <nav class="navbar">
    <div class="navbar-inner">
      <a href="/" class="navbar-logo">AI 写作助手</a>
      <ul class="navbar-links">
        <li><a href="/">首页</a></li>
        <li><a href="/tools/weekly">工具</a></li>
        <li><a href="/console">控制台</a></li>
        <li><a href="/pricing">价格</a></li>
      </ul>
      <div class="navbar-usage">
        <span class="usage-badge">剩余 10 次</span>
      </div>
    </div>
  </nav>
  <main class="main-content">
    <div class="container">
      <div style="padding:24px 0 0">
        <h1 style="font-size:28px;font-weight:800;color:var(--text-primary)">周报生成器</h1>
        <p style="color:var(--text-secondary);margin-top:4px">输入本周工作内容，一键生成专业周报</p>
      </div>
      <div class="tool-layout">
        <div class="tool-section">
          <h2>输入信息</h2>
          <div class="form-group">
            <label for="content">本周完成的工作内容</label>
            <textarea id="content" data-param="content" placeholder="本周完成的工作内容...例如：完成了用户管理模块的开发"></textarea>
          </div>
          <div class="form-group">
            <label for="highlights">本周亮点</label>
            <input id="highlights" data-param="highlights" placeholder="例如：性能优化提升 30%">
          </div>
          <div class="form-group">
            <label for="nextPlan">下周计划</label>
            <input id="nextPlan" data-param="nextPlan" placeholder="例如：开始订单系统的开发">
          </div>
          <button id="generateBtn" class="btn btn-primary btn-block btn-lg" data-type="weekly">生成</button>
        </div>
        <div class="tool-section">
          <div class="output-header">
            <h2>生成结果</h2>
            <button id="copyBtn" class="copy-btn">复制文案</button>
          </div>
          <div id="outputArea" class="output-area" aria-live="polite"></div>
          <p style="font-size:13px;color:var(--text-muted)">每次生成消耗 1 次免费次数</p>
        </div>
      </div>
    </div>
  </main>
  <footer class="footer">
    <div class="container">
      <p>&copy; 2026 AI 写作助手. All rights reserved.</p>
    </div>
  </footer>
  <script src="/js/api.js"></script>
  <script src="/js/app.js"></script>
</body>
</html>`,

  "/tools/xiaohongshu": `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="description" content="AI 写作助手 - 一键生成周报、小红书文案、朋友圈文案，智能 AI 写作工具">
  <meta property="og:title" content="AI 写作助手 - 一键生成优质内容">
  <meta property="og:description" content="周报 · 小红书 · 朋友圈，一键生成">
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>✍️</text></svg>">
  <title>小红书文案生成器 - AI 写作助手</title>
  <link rel="stylesheet" href="/css/style.css">
</head>
<body>
  <nav class="navbar">
    <div class="navbar-inner">
      <a href="/" class="navbar-logo">AI 写作助手</a>
      <ul class="navbar-links">
        <li><a href="/">首页</a></li>
        <li><a href="/tools/weekly">工具</a></li>
        <li><a href="/console">控制台</a></li>
        <li><a href="/pricing">价格</a></li>
      </ul>
      <div class="navbar-usage">
        <span class="usage-badge">剩余 10 次</span>
      </div>
    </div>
  </nav>
  <main class="main-content">
    <div class="container">
      <div class="tool-header">
        <h1>小红书文案生成器</h1>
        <p>智能生成小红书爆款文案，标题吸睛、正文动人</p>
      </div>
      <div class="tool-layout">
        <div class="tool-section">
          <h2>输入信息</h2>
          <div class="form-group">
            <label for="topic">主题</label>
            <input id="topic" data-param="topic" placeholder="例如：一款好用的笔记软件推荐">
          </div>
          <div class="form-group">
            <label for="audience">目标受众</label>
            <select id="audience" data-param="audience">
              <option value="通用">通用</option>
              <option value="学生党">学生党</option>
              <option value="上班族">上班族</option>
              <option value="宝妈">宝妈</option>
              <option value="健身爱好者">健身爱好者</option>
              <option value="美妆爱好者">美妆爱好者</option>
            </select>
          </div>
          <div class="form-group">
            <label for="tone">语气风格</label>
            <select id="tone" data-param="tone">
              <option value="活泼">活泼</option>
              <option value="正式">正式</option>
              <option value="专业">专业</option>
              <option value="幽默">幽默</option>
              <option value="温暖">温暖</option>
            </select>
          </div>
          <button id="generateBtn" class="btn btn-primary btn-block btn-lg" data-type="xiaohongshu">生成</button>
        </div>
        <div class="tool-section">
          <div class="output-header">
            <h2>生成结果</h2>
            <button id="copyBtn" class="copy-btn">复制文案</button>
          </div>
          <div id="outputArea" class="output-area" aria-live="polite"></div>
          <p style="font-size:13px;color:var(--text-muted)">每次生成消耗 1 次免费次数</p>
        </div>
      </div>
    </div>
  </main>
  <footer class="footer">
    <div class="container">
      <p>&copy; 2026 AI 写作助手. All rights reserved.</p>
    </div>
  </footer>
  <script src="/js/api.js"></script>
  <script src="/js/app.js"></script>
</body>
</html>`,

  "/tools/moments": `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta name="description" content="AI 写作助手 - 一键生成周报、小红书文案、朋友圈文案，智能 AI 写作工具">
  <meta property="og:title" content="AI 写作助手 - 一键生成优质内容">
  <meta property="og:description" content="周报 · 小红书 · 朋友圈，一键生成">
  <link rel="icon" href="data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='.9em' font-size='90'>✍️</text></svg>">
  <title>朋友圈文案生成器 - AI 写作助手</title>
  <link rel="stylesheet" href="/css/style.css">
</head>
<body>
  <nav class="navbar">
    <div class="navbar-inner">
      <a href="/" class="navbar-logo">AI 写作助手</a>
      <ul class="navbar-links">
        <li><a href="/">首页</a></li>
        <li><a href="/tools/weekly">工具</a></li>
        <li><a href="/console">控制台</a></li>
        <li><a href="/pricing">价格</a></li>
      </ul>
      <div class="navbar-usage">
        <span class="usage-badge">剩余 10 次</span>
      </div>
    </div>
  </nav>
  <main class="main-content">
    <div class="container">
      <div class="tool-header">
        <h1>朋友圈文案生成器</h1>
        <p>根据场景和心情生成朋友圈文案</p>
      </div>
      <div class="tool-layout">
        <div class="tool-section">
          <h2>输入信息</h2>
          <div class="form-group">
            <label for="content">想表达的内容</label>
            <textarea id="content" data-param="content" placeholder="描述你想表达的内容或心情..."></textarea>
          </div>
          <div class="form-group">
            <label for="scene">场景选择</label>
            <select id="scene" data-param="scene">
              <option value="work">工作</option>
              <option value="life">生活</option>
              <option value="holiday">节日</option>
              <option value="other">其他</option>
            </select>
          </div>
          <button id="generateBtn" class="btn btn-primary btn-block btn-lg" data-type="moments">生成</button>
        </div>
        <div class="tool-section">
          <div class="output-header">
            <h2>生成结果</h2>
            <button id="copyBtn" class="copy-btn">复制文案</button>
          </div>
          <div id="outputArea" class="output-area" aria-live="polite"></div>
          <p style="font-size:13px;color:var(--text-muted)">每次生成消耗 1 次免费次数</p>
        </div>
      </div>
    </div>
  </main>
  <footer class="footer">
    <div class="container">
      <p>&copy; 2026 AI 写作助手. All rights reserved.</p>
    </div>
  </footer>
  <script src="/js/api.js"></script>
  <script src="/js/app.js"></script>
</body>
</html>`,

  "/css/style.css": `:root {
  --bg-primary: #171717;
  --bg-secondary: #1a1a1a;
  --bg-card: #1f1f1f;
  --text-primary: #f7f7f7;
  --text-secondary: #999;
  --text-muted: #666;
  --accent: #17f700;
  --accent-hover: #14d400;
  --accent-light: rgba(23,247,0,0.12);
  --accent-glow: rgba(23,247,0,0.08);
  --border: #2a2a2a;
  --border-light: #333;
  --radius: 12px;
  --radius-sm: 6px;
  --font-sans: -apple-system, BlinkMacSystemFont, "Microsoft YaHei", "PingFang SC", sans-serif;
  --font-mono: "SF Mono", SFMono-Regular, monospace;
}
* { margin: 0; padding: 0; box-sizing: border-box; }
html { scroll-behavior: smooth; }
body {
  font-family: var(--font-sans);
  background: var(--bg-primary);
  color: var(--text-primary);
  line-height: 1.7;
  min-height: 100vh;
  display: flex;
  flex-direction: column;
  -webkit-font-smoothing: antialiased;
}

body::before,
body::after {
  content: '';
  position: fixed;
  border-radius: 50%;
  filter: blur(120px);
  pointer-events: none;
  z-index: 0;
}
body::before {
  width: 600px;
  height: 600px;
  background: rgba(23,247,0,0.05);
  top: -200px;
  right: -200px;
}
body::after {
  width: 400px;
  height: 400px;
  background: rgba(23,247,0,0.03);
  bottom: -100px;
  left: -100px;
}

.navbar {
  position: fixed;
  top: 0;
  left: 0;
  right: 0;
  z-index: 100;
  background: transparent;
  border-bottom: 1px solid var(--border);
  height: 56px;
}
.navbar-inner {
  max-width: 1200px;
  margin: 0 auto;
  padding: 0 24px;
  height: 100%;
  display: flex;
  align-items: center;
  justify-content: space-between;
}
.navbar-logo {
  font-size: 16px;
  font-weight: 700;
  color: var(--text-primary);
  text-decoration: none;
  letter-spacing: -0.3px;
}
.navbar-links {
  display: flex;
  align-items: center;
  gap: 24px;
  list-style: none;
}
.navbar-links a {
  text-decoration: none;
  color: #666;
  font-size: 13px;
  font-weight: 500;
  transition: color .2s;
  padding: 4px 0;
}
.navbar-links a:hover { color: #fff; }
.navbar-links a.active { color: var(--text-primary); }
.navbar-usage { display: flex; align-items: center; gap: 12px; }
.usage-badge {
  font-size: 12px;
  color: var(--text-secondary);
  padding: 3px 10px;
  border: 1px solid var(--border);
  border-radius: 20px;
  font-weight: 500;
}
.main-content { flex: 1; padding-top: 56px; position: relative; z-index: 1; }
.container { max-width: 1000px; margin: 0 auto; padding: 0 24px; }

.hero { padding: 120px 0 80px; text-align: center; }
.hero h1 {
  font-size: 56px;
  font-weight: 800;
  color: var(--text-primary);
  letter-spacing: -2px;
  margin-bottom: 16px;
  line-height: 1.1;
}
.hero p {
  font-size: 17px;
  color: var(--text-secondary);
  max-width: 440px;
  margin: 0 auto 40px;
  line-height: 1.6;
}
.hero-scroll {
  margin-top: 60px;
  font-size: 13px;
  color: var(--text-muted);
  opacity: 0.5;
  animation: float 2s ease-in-out infinite;
}
@keyframes float {
  0%, 100% { transform: translateY(0); }
  50% { transform: translateY(6px); }
}

.btn {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  gap: 6px;
  padding: 12px 28px;
  border-radius: var(--radius);
  font-size: 14px;
  font-weight: 600;
  cursor: pointer;
  transition: all .25s;
  border: none;
  text-decoration: none;
  font-family: inherit;
}
.btn-primary {
  background: var(--accent);
  color: #fff;
}
.btn-primary:hover {
  background: var(--accent-hover);
  box-shadow: 0 0 30px var(--accent-glow);
}
.btn-outline {
  background: transparent;
  color: var(--text-primary);
  border: 1px solid var(--border);
}
.btn-outline:hover {
  border-color: var(--text-primary);
  background: rgba(255,255,255,0.05);
}
.btn-secondary {
  background: var(--bg-card);
  color: var(--text-secondary);
  border: 1px solid var(--border);
}
.btn-secondary:hover {
  border-color: var(--text-muted);
  color: var(--text-primary);
}
.btn-lg { padding: 14px 34px; font-size: 16px; }
.btn-block { width: 100%; }
.btn:disabled { opacity: .3; cursor: not-allowed; }

.card-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
  gap: 20px;
  padding: 16px 0 100px;
}
.card {
  background: var(--bg-card);
  border-radius: var(--radius);
  padding: 32px;
  border: 1px solid var(--border);
  transition: border-color .25s;
}
.card:hover { border-color: var(--text-muted); }
.card-icon {
  width: 40px;
  height: 40px;
  border-radius: var(--radius-sm);
  background: var(--accent-light);
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 18px;
  margin-bottom: 16px;
}
.card h3 {
  font-size: 18px;
  font-weight: 700;
  color: var(--text-primary);
  margin-bottom: 8px;
}
.card p {
  color: var(--text-secondary);
  font-size: 14px;
  margin-bottom: 20px;
  line-height: 1.6;
}
.card-usage { font-size: 13px; color: var(--text-muted); margin-bottom: 20px; }
.card-usage .free { color: var(--accent); font-weight: 600; }

.tool-header { padding: 32px 0 0; }
.tool-header h1 { font-size: 28px; font-weight: 800; color: var(--text-primary); }
.tool-header p { color: var(--text-secondary); margin-top: 4px; font-size: 15px; }
.tool-layout {
  display: grid;
  grid-template-columns: 1fr 1fr;
  gap: 28px;
  padding: 32px 0 100px;
}
.tool-section {
  background: var(--bg-card);
  border-radius: var(--radius);
  padding: 28px 32px;
  border: 1px solid var(--border);
}
.tool-section h2 {
  font-size: 16px;
  font-weight: 700;
  color: var(--text-primary);
  margin-bottom: 20px;
  padding-bottom: 12px;
  border-bottom: 1px solid var(--border);
}
.form-group { margin-bottom: 20px; }
.form-group label {
  display: block;
  font-size: 13px;
  font-weight: 600;
  color: var(--text-secondary);
  margin-bottom: 6px;
}
.form-group textarea,
.form-group input,
.form-group select {
  width: 100%;
  padding: 10px 14px;
  border: 1px solid var(--border);
  border-radius: var(--radius-sm);
  font-size: 14px;
  font-family: inherit;
  color: var(--text-primary);
  background: var(--bg-secondary);
  transition: border-color .2s;
  outline: none;
}
.form-group textarea:focus,
.form-group input:focus,
.form-group select:focus {
  border-color: var(--accent);
}
.form-group textarea { min-height: 130px; resize: vertical; }
.output-area {
  min-height: 200px;
  background: var(--bg-secondary);
  border-radius: var(--radius-sm);
  padding: 20px;
  white-space: pre-wrap;
  line-height: 1.7;
  font-size: 14px;
  color: var(--text-primary);
  border: 1px solid var(--border);
  margin-bottom: 16px;
}
.output-area:empty::before {
  content: "生成的文案将显示在这里";
  color: var(--text-muted);
}
.output-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 12px;
}
.copy-btn {
  display: inline-flex;
  align-items: center;
  gap: 5px;
  padding: 7px 14px;
  border-radius: var(--radius-sm);
  font-size: 13px;
  font-weight: 500;
  color: var(--text-secondary);
  background: transparent;
  border: 1px solid var(--border);
  cursor: pointer;
  transition: all .2s;
  font-family: inherit;
}
.copy-btn:hover { border-color: var(--text-muted); color: var(--text-primary); }
.copy-btn.copied { background: var(--accent-light); color: var(--accent); border-color: var(--accent); }

.pricing-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
  gap: 20px;
  padding: 32px 0 100px;
}
.pricing-card {
  background: var(--bg-card);
  border-radius: var(--radius);
  padding: 36px 28px;
  border: 1px solid var(--border);
  text-align: center;
  transition: border-color .25s;
}
.pricing-card:hover { border-color: var(--text-muted); }
.pricing-card.featured {
  border-color: var(--accent);
  box-shadow: 0 0 0 1px var(--accent), 0 0 40px var(--accent-glow);
}
.pricing-card .badge {
  display: inline-block;
  font-size: 11px;
  font-weight: 700;
  letter-spacing: .8px;
  padding: 3px 10px;
  border-radius: 20px;
  margin-bottom: 16px;
}
.pricing-card .badge.popular {
  background: var(--accent-light);
  color: var(--accent);
}
.pricing-card .badge.coming {
  background: rgba(255,255,255,0.05);
  color: var(--text-muted);
}
.pricing-card .plan-name { font-size: 16px; font-weight: 700; color: var(--text-primary); margin-bottom: 6px; }
.pricing-card .plan-price { font-size: 40px; font-weight: 800; color: var(--text-primary); margin-bottom: 4px; }
.pricing-card .plan-price .unit { font-size: 15px; font-weight: 500; color: var(--text-secondary); }
.pricing-card .plan-desc { font-size: 14px; color: var(--text-secondary); margin-bottom: 20px; }
.pricing-card .plan-features { list-style: none; margin-bottom: 28px; }
.pricing-card .plan-features li { padding: 8px 0; font-size: 14px; color: var(--text-secondary); border-bottom: 1px solid var(--border); }
.pricing-card .plan-features li:last-child { border-bottom: none; }

.footer {
  border-top: 1px solid var(--border);
  padding: 24px 0;
  text-align: center;
  color: var(--text-muted);
  font-size: 13px;
  position: relative;
  z-index: 1;
}

@media (max-width: 768px) {
  .hero { padding: 80px 0 48px; }
  .hero h1 { font-size: 34px; }
  .hero p { font-size: 15px; }
  .tool-layout { grid-template-columns: 1fr; }
  .navbar-inner { padding: 0 16px; }
  .navbar-links { gap: 14px; }
  .navbar-links a { font-size: 12px; }
  .card-grid, .pricing-grid { grid-template-columns: 1fr; }
  .container { padding: 0 16px; }
  .tool-section { padding: 20px; }
}

.console-page{background:#0a0e14;height:100vh;padding-top:0;overflow:hidden;font-family:'SF Mono',SFMono-Regular,'Cascadia Code','Fira Code',monospace}.console-page .navbar{display:none}.console-page .main-content{padding-top:0}.console-page .footer{display:none}.console-win{display:flex;flex-direction:column;height:100vh;background:#0a0e14}.console-titlebar{display:flex;align-items:center;justify-content:space-between;background:#1a1f2a;padding:8px 16px;border-bottom:1px solid #2a3344;user-select:none;flex-shrink:0}.console-titlebar-title{display:flex;align-items:center;gap:8px;font-size:13px;color:#e8ecf1;font-weight:500}.console-titlebar-dots{display:flex;gap:8px}.console-titlebar-dot{width:13px;height:13px;border-radius:50%;cursor:pointer;transition:opacity .2s}.console-titlebar-dot:hover{opacity:.8}.console-titlebar-dot.red{background:#ff5f57}.console-titlebar-dot.yellow{background:#ffbd2e}.console-titlebar-dot.green{background:#28c840}.console-body{display:flex;flex:1;overflow:hidden}.console-sidebar{width:200px;background:#131820;border-right:1px solid #2a3344;padding:12px 0;flex-shrink:0;overflow-y:auto}.console-sidebar-item{display:flex;align-items:center;gap:10px;padding:10px 20px;font-size:13px;color:#6a7a8e;cursor:pointer;transition:all .15s;border-left:3px solid transparent;font-family:var(--font-sans);text-decoration:none}.console-sidebar-item:hover{color:#e8ecf1;background:rgba(255,255,255,0.03)}.console-sidebar-item.active{color:#17f700;border-left-color:#17f700;background:rgba(23,247,0,0.05)}.console-sidebar-item .icon{font-size:15px;width:20px;text-align:center}.console-main{flex:1;padding:24px 32px;overflow-y:auto;background:#0a0e14}.console-main h2{font-size:15px;font-weight:600;color:#e8ecf1;margin-bottom:4px;letter-spacing:.5px;text-transform:uppercase}.console-main .subtitle{font-size:12px;color:#6a7a8e;margin-bottom:24px}.console-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px;margin-bottom:28px}.console-card{background:#131820;border:1px solid #2a3344;border-radius:8px;padding:18px 20px;transition:border-color .2s}.console-card:hover{border-color:#3a4a5e}.console-card .label{font-size:11px;color:#6a7a8e;text-transform:uppercase;letter-spacing:.8px;margin-bottom:8px;font-weight:600}.console-card .value{font-size:22px;font-weight:700;color:#e8ecf1;font-family:'SF Mono',SFMono-Regular,monospace}.console-card .value.green{color:#17f700}.console-card .value.yellow{color:#f0c239}.console-card .value.red{color:#ff5f57}.console-card .value.blue{color:#56d4e0}.console-card .sub{font-size:11px;color:#6a7a8e;margin-top:4px}.console-section-title{font-size:13px;font-weight:600;color:#e8ecf1;margin-bottom:16px;padding-bottom:8px;border-bottom:1px solid #2a3344;display:flex;align-items:center;gap:8px}.console-log{background:#0c111a;border:1px solid #2a3344;border-radius:6px;padding:16px;font-size:12px;line-height:1.8;max-height:300px;overflow-y:auto;font-family:'SF Mono',SFMono-Regular,monospace}.console-log-entry{display:flex;gap:12px;padding:3px 0;border-bottom:1px solid rgba(42,51,68,0.3)}.console-log-entry:last-child{border-bottom:none}.console-log-time{color:#6a7a8e;flex-shrink:0;width:80px}.console-log-level{flex-shrink:0;width:50px;font-weight:600}.console-log-level.info{color:#56d4e0}.console-log-level.success{color:#17f700}.console-log-level.warn{color:#f0c239}.console-log-level.error{color:#ff5f57}.console-log-msg{color:#a8b2c1}.console-status-bar{display:flex;align-items:center;gap:16px;padding:8px 20px;background:#1a1f2a;border-top:1px solid #2a3344;font-size:11px;color:#6a7a8e;flex-shrink:0;font-family:var(--font-sans)}.console-status-bar .status-dot{width:7px;height:7px;border-radius:50%;display:inline-block}.console-status-bar .status-dot.green{background:#17f700}.console-status-bar .status-dot.red{background:#ff5f57}.console-status-bar .status-dot.yellow{background:#f0c239}.console-btn{display:inline-flex;align-items:center;gap:6px;padding:6px 14px;border-radius:4px;font-size:12px;font-weight:500;cursor:pointer;transition:all .15s;border:1px solid #2a3344;background:transparent;color:#a8b2c1;font-family:var(--font-sans)}.console-btn:hover{border-color:#3a4a5e;color:#e8ecf1;background:rgba(255,255,255,0.03)}.console-btn.primary{background:#17f700;color:#0a0e14;border-color:#17f700;font-weight:600}.console-btn.primary:hover{background:#14d400}.console-toolbar{display:flex;gap:8px;margin-bottom:16px;flex-wrap:wrap}.console-welcome{text-align:center;padding:60px 20px}.console-welcome .greeting{font-size:28px;font-weight:700;color:#e8ecf1;margin-bottom:8px}.console-welcome .greeting span{color:#17f700}.console-welcome p{color:#6a7a8e;font-size:14px;max-width:500px;margin:0 auto;line-height:1.7}
`,

  "/js/api.js": `const API_CONFIG={baseUrl:"/api/generate",timeout:30000};const MOCK_RESPONSES={weekly:(p)=>{return"【本周工作汇报】\\n\\n一、本周完成工作\\n"+(p.content||"未提供")+"\\n\\n二、本周工作亮点\\n"+(p.highlights||"暂无")+"\\n\\n三、下周工作计划\\n"+(p.nextPlan||"暂无")+"\\n\\n四、总结\\n各项工作按计划推进。"},xiaohongshu:(p)=>{const t=p.topic||"默认主题";const titles=[""+t+"的使用体验分享🔥","我不允许还有人不知道"+t+"✨","强烈推荐"+t+"太好用了",""+t+"保姆级攻略来了📖","后悔没早知道的"+t+"，建议收藏⭐"];return"📌 标题："+titles[Math.floor(Math.random()*titles.length)]+"\\n\\n📝 正文：\\n姐妹们！今天一定要跟大家分享一下"+t+"的宝藏用法✨\\n\\n真的真的强烈推荐！谁用谁知道！😎\\n\\n#"+t.replace(/\\s+/g,"")+" #好物分享 #宝藏推荐"},moments:(p)=>{const c=p.content||"今天的心情";return c+"\\n\\n生活明朗，万物可爱 ✨\\n#日常生活 #小确幸\\n\\n📷 配图建议：生活照片 / 风景照 / 美食图"}};async function generateContent(type,params){try{const c=new AbortController;const t=setTimeout(()=>c.abort(),API_CONFIG.timeout);const r=await fetch(API_CONFIG.baseUrl,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type,params}),signal:c.signal});clearTimeout(t);if(!r.ok)throw new Error("HTTP "+r.status);const d=await r.json();return d.content||d}catch(e){console.warn("API call failed, using mock: "+e.message);const f=MOCK_RESPONSES[type];if(!f)throw new Error("Unknown type: "+type);await new Promise(r=>setTimeout(r,800+Math.random()*600));return f(params)}}`,

  "/js/app.js": `const USAGE_KEY="free_usage_remaining";const DEFAULT_FREE_TRIALS=10;function getUsageRemaining(){const v=localStorage.getItem(USAGE_KEY);if(v===null){localStorage.setItem(USAGE_KEY,DEFAULT_FREE_TRIALS);return DEFAULT_FREE_TRIALS}return parseInt(v,10)||0}function decrementUsage(){const c=getUsageRemaining();const n=Math.max(0,c-1);localStorage.setItem(USAGE_KEY,n);updateUsageUI();return n}function updateUsageUI(){const r=getUsageRemaining();document.querySelectorAll(".usage-badge").forEach(el=>{el.textContent="剩余 "+r+" 次"});document.querySelectorAll(".free-count").forEach(el=>{el.textContent=r});if(r<=0){document.querySelectorAll(".btn.btn-primary[data-type]").forEach(btn=>{btn.disabled=true;btn.title="免费次数已用完，请前往价格页购买"})}}function initNavbar(){const p=window.location.pathname;document.querySelectorAll(".navbar-links a").forEach(a=>{const h=a.getAttribute("href");if(!h)return;if(p===h||p.endsWith(h)||(h==="/"&&(p==="/"||p==="/index.html")))a.classList.add("active")})}function initToolPage(){const btn=document.getElementById("generateBtn");const out=document.getElementById("outputArea");const copy=document.getElementById("copyBtn");if(!btn||!out)return;btn.addEventListener("click",async()=>{if(getUsageRemaining()<=0){alert("免费次数已用完，请前往价格页购买");window.location.href="/pricing";return}btn.disabled=true;btn.textContent="生成中...";try{const params={};document.querySelectorAll("[data-param]").forEach(el=>{params[el.dataset.param]=el.value});const content=await generateContent(btn.dataset.type||"weekly",params);out.textContent=content;out.dataset.content=content;decrementUsage();if(copy){copy.classList.remove("copied");copy.textContent="复制文案"}}catch(err){out.textContent="生成失败："+err.message}finally{btn.disabled=false;btn.textContent="生成"}});if(copy){copy.addEventListener("click",()=>{const c=out.dataset.content||out.textContent;if(!c||c==="生成的文案将显示在这里"){alert("暂无内容可复制");return}navigator.clipboard.writeText(c).then(()=>{copy.classList.add("copied");copy.textContent="已复制";setTimeout(()=>{copy.classList.remove("copied");copy.textContent="复制文案"},2000)}).catch(()=>{const ta=document.createElement("textarea");ta.value=c;document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);copy.classList.add("copied");copy.textContent="已复制";setTimeout(()=>{copy.classList.remove("copied");copy.textContent="复制文案"},2000)})})}}document.addEventListener("DOMContentLoaded",()=>{initNavbar();updateUsageUI();initToolPage()})`,

  "/console": `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>控制台 - AI 写作助手</title>
  <link rel="stylesheet" href="/css/style.css">
</head>
<body class="console-page">
  <div class="console-win">
    <div class="console-titlebar">
      <div class="console-titlebar-title">
        <span style="font-weight:700;color:#17f700">◆</span>
        AI Writer Console v1.0
      </div>
      <div class="console-titlebar-dots">
        <span class="console-titlebar-dot yellow"></span>
        <span class="console-titlebar-dot green"></span>
        <span class="console-titlebar-dot red"></span>
      </div>
    </div>
    <div class="console-body">
      <nav class="console-sidebar">
        <a class="console-sidebar-item active" href="#" data-section="overview"><span class="icon">📊</span> 总览</a>
        <a class="console-sidebar-item" href="#" data-section="api"><span class="icon">🔌</span> API 状态</a>
        <a class="console-sidebar-item" href="#" data-section="usage"><span class="icon">📈</span> 用量统计</a>
        <a class="console-sidebar-item" href="#" data-section="logs"><span class="icon">📋</span> 操作日志</a>
        <a class="console-sidebar-item" href="#" data-section="tools"><span class="icon">⚡</span> 快速工具</a>
        <a style="margin-top:auto;border-top:1px solid #2a3344;padding-top:12px" class="console-sidebar-item" href="/"><span class="icon">←</span> 返回网站</a>
      </nav>
      <main class="console-main" id="consoleMain">
        <div class="console-welcome" id="section-overview">
          <div class="greeting">欢迎回来, <span>管理员</span></div>
          <p>AI Writer 控制台 v1.0 — 实时监控网站运行状态、API 用量和系统健康度</p>
        </div>
        <div class="console-grid" id="overviewCards">
          <div class="console-card"><div class="label">网站状态</div><div class="value green" id="ovStatus">检查中...</div><div class="sub" id="ovStatusSub">正在检测</div></div>
          <div class="console-card"><div class="label">部署地址</div><div class="value" style="font-size:14px" id="ovUrl">-</div><div class="sub">Cloudflare Pages</div></div>
          <div class="console-card"><div class="label">剩余免费次数</div><div class="value" id="ovFree">-</div><div class="sub">/ 10 次总量</div></div>
          <div class="console-card"><div class="label">今日生成</div><div class="value" id="ovToday">0</div><div class="sub" id="ovTodaySub">次 AI 调用</div></div>
        </div>
        <div class="console-section-title">📌 快速入口</div>
        <div class="console-toolbar">
          <button class="console-btn primary" onclick="window.location.href='/tools/weekly'">✏️ 写周报</button>
          <button class="console-btn primary" onclick="window.location.href='/tools/xiaohongshu'">📕 小红书</button>
          <button class="console-btn primary" onclick="window.location.href='/tools/moments'">💬 朋友圈</button>
          <button class="console-btn" onclick="window.location.href='/pricing'">💰 价格页</button>
        </div>
      </main>
    </div>
    <div class="console-status-bar">
      <span class="status-dot green" id="statusDot"></span>
      <span id="statusText">系统运行中</span>
      <span style="margin-left:auto">部署: master.ai-writer-3rm.pages.dev</span>
      <span id="statusTime"></span>
    </div>
  </div>
  <script>
const SECTIONS = ['overview','api','usage','logs','tools'];
const SECTION_CONTENT = {
  overview: \`<div class="console-welcome" id="section-overview"><div class="greeting">欢迎回来, <span>管理员</span></div><p>AI Writer 控制台 v1.0 — 实时监控网站运行状态、API 用量和系统健康度</p></div><div class="console-grid" id="overviewCards"><div class="console-card"><div class="label">网站状态</div><div class="value green" id="ovStatus">检查中...</div><div class="sub" id="ovStatusSub">正在检测</div></div><div class="console-card"><div class="label">部署地址</div><div class="value" style="font-size:14px" id="ovUrl">-</div><div class="sub">Cloudflare Pages</div></div><div class="console-card"><div class="label">剩余免费次数</div><div class="value" id="ovFree">-</div><div class="sub">/ 10 次总量</div></div><div class="console-card"><div class="label">今日生成</div><div class="value" id="ovToday">0</div><div class="sub" id="ovTodaySub">次 AI 调用</div></div></div><div class="console-section-title">📌 快速入口</div><div class="console-toolbar"><button class="console-btn primary" onclick="window.location.href='/tools/weekly'">✏️ 写周报</button><button class="console-btn primary" onclick="window.location.href='/tools/xiaohongshu'">📕 小红书</button><button class="console-btn primary" onclick="window.location.href='/tools/moments'">💬 朋友圈</button><button class="console-btn" onclick="window.location.href='/pricing'">💰 价格页</button></div>\`,
  api: \`<h2>🔌 API 状态</h2><p class="subtitle">DeepSeek API 连接状态和响应时间</p><div class="console-grid"><div class="console-card"><div class="label">API 端点</div><div class="value" style="font-size:13px">api.deepseek.com</div><div class="sub">DeepSeek Chat</div></div><div class="console-card"><div class="label">连接状态</div><div class="value green" id="apiStatus">检查中...</div><div class="sub">通过 Cloudflare Workers</div></div><div class="console-card"><div class="label">响应时间</div><div class="value" id="apiLatency">-</div><div class="sub">上次测试</div></div><div class="console-card"><div class="label">模型</div><div class="value" style="font-size:14px">deepseek-chat</div><div class="sub">DeepSeek V4</div></div></div><div class="console-toolbar"><button class="console-btn" onclick="testConnection()">🔄 测试连接</button></div><div id="apiResult" style="margin-top:12px;padding:12px;background:#0c111a;border:1px solid #2a3344;border-radius:6px;font-size:12px;color:#6a7a8e;display:none"></div>\`,
  usage: \`<h2>📈 用量统计</h2><p class="subtitle">AI 写作助手使用情况概览</p><div class="console-grid"><div class="console-card"><div class="label">总生成次数</div><div class="value" id="usTotal">0</div><div class="sub">累计使用</div></div><div class="console-card"><div class="label">免费剩余</div><div class="value green" id="usFree">-</div><div class="sub">/ 10 次</div></div><div class="console-card"><div class="label">本周生成</div><div class="value" id="usWeek">0</div><div class="sub">本周统计</div></div><div class="console-card"><div class="label">今日生成</div><div class="value" id="usToday">0</div><div class="sub">今日统计</div></div></div><div class="console-section-title">📊 使用趋势</div><div style="background:#131820;border:1px solid #2a3344;border-radius:8px;padding:20px;text-align:center;color:#6a7a8e;font-size:13px">使用数据存储在本地浏览器，清空浏览器会重置统计</div>\`,
  logs: \`<h2>📋 操作日志</h2><p class="subtitle">最近的 AI 生成记录</p><div class="console-log" id="logContainer"><div style="color:#6a7a8e;text-align:center;padding:40px 0">暂无日志记录</div></div><div class="console-toolbar" style="margin-top:12px"><button class="console-btn" onclick="clearLogs()">🗑️ 清空日志</button></div>\`,
  tools: \`<h2>⚡ 快速工具</h2><p class="subtitle">网站运维小工具</p><div class="console-grid"><div class="console-card"><div class="label">网站连通性</div><div class="value green" id="tcStatus">正常</div><div class="sub"><button class="console-btn" style="margin-top:8px" onclick="testConnection()">🔄 测试</button></div></div><div class="console-card"><div class="label">本地存储</div><div class="value" id="lcSize" style="font-size:16px">计算中...</div><div class="sub"><button class="console-btn" style="margin-top:8px" onclick="calcStorage()">📊 查看</button></div></div><div class="console-card"><div class="label">重置数据</div><div class="value" style="font-size:13px;font-weight:400;color:#6a7a8e">清除所有本地数据</div><div class="sub"><button class="console-btn" onclick="resetData()" style="color:#ff5f57;border-color:#ff5f57;margin-top:8px">⚠️ 重置</button></div></div><div class="console-card"><div class="label">导出日志</div><div class="value" style="font-size:13px;font-weight:400;color:#6a7a8e">下载操作日志备份</div><div class="sub"><button class="console-btn" style="margin-top:8px" onclick="exportLogs()">📥 导出</button></div></div></div>\`
};

function switchSection(id) {
  document.querySelectorAll('.console-sidebar-item').forEach(el => el.classList.remove('active'));
  document.querySelector(\`[data-section="\${id}"]\`)?.classList.add('active');
  const main = document.getElementById('consoleMain');
  main.innerHTML = SECTION_CONTENT[id] || '<div style="color:#6a7a8e;text-align:center;padding:60px">建设中...</div>';
  if (id === 'overview') initOverview();
  if (id === 'usage') initUsage();
  if (id === 'logs') initLogs();
}

function initOverview() {
  try {
    const remaining = parseInt(localStorage.getItem('free_usage_remaining') || '10', 10);
    document.getElementById('ovFree').textContent = remaining;
  } catch(e) {}
  const today = new Date().toDateString();
  const logs = JSON.parse(localStorage.getItem('aiwriter_logs') || '[]');
  const todayCount = logs.filter(l => new Date(l.time).toDateString() === today).length;
  document.getElementById('ovToday').textContent = todayCount;
  const url = window.location.origin;
  document.getElementById('ovUrl').textContent = url;
  fetch('/api/usage').then(r => r.json()).then(d => {
    document.getElementById('ovStatus').textContent = '运行正常';
    document.getElementById('ovStatus').className = 'value green';
    document.getElementById('ovStatusSub').textContent = 'API 可用';
  }).catch(() => {
    document.getElementById('ovStatus').textContent = '异常';
    document.getElementById('ovStatus').className = 'value red';
    document.getElementById('ovStatusSub').textContent = 'API 不可用';
  });
}

function initUsage() {
  try {
    const remaining = parseInt(localStorage.getItem('free_usage_remaining') || '10', 10);
    document.getElementById('usFree').textContent = remaining;
    const total = 10 - remaining;
    document.getElementById('usTotal').textContent = total;
    const logs = JSON.parse(localStorage.getItem('aiwriter_logs') || '[]');
    const now = new Date();
    const today = now.toDateString();
    const weekStart = new Date(now); weekStart.setDate(now.getDate() - now.getDay());
    document.getElementById('usToday').textContent = logs.filter(l => new Date(l.time).toDateString() === today).length;
    document.getElementById('usWeek').textContent = logs.filter(l => new Date(l.time) >= weekStart).length;
  } catch(e) {}
}

function initLogs() {
  try {
    const logs = JSON.parse(localStorage.getItem('aiwriter_logs') || '[]');
    const container = document.getElementById('logContainer');
    if (!logs.length) { container.innerHTML = '<div style="color:#6a7a8e;text-align:center;padding:40px 0">暂无日志记录</div>'; return; }
    container.innerHTML = logs.reverse().map(l => {
      const time = new Date(l.time).toLocaleString();
      const level = l.success ? 'success' : 'error';
      const label = l.success ? '成功' : '失败';
      const typeMap = {weekly:'周报',xiaohongshu:'小红书',moments:'朋友圈'};
      const type = typeMap[l.type] || l.type;
      return '<div class="console-log-entry"><span class="console-log-time">' + time + '</span><span class="console-log-level ' + level + '">' + label + '</span><span class="console-log-msg">[' + type + '] ' + (l.topic || '') + '</span></div>';
    }).join('');
  } catch(e) {}
}

function testConnection() {
  const el = document.getElementById('apiResult');
  if (el) { el.style.display = 'block'; el.textContent = '测试中...'; el.style.color = '#f0c239'; }
  document.getElementById('apiStatus').textContent = '测试中...';
  const start = Date.now();
  fetch('/api/usage').then(r => r.json()).then(d => {
    const ms = Date.now() - start;
    document.getElementById('apiStatus').textContent = '正常';
    document.getElementById('apiStatus').className = 'value green';
    document.getElementById('apiLatency').textContent = ms + 'ms';
    if (el) { el.textContent = '✅ API 连接成功 | 响应时间: ' + ms + 'ms | 剩余免费: ' + (d.remaining || '?'); el.style.color = '#17f700'; }
  }).catch(e => {
    document.getElementById('apiStatus').textContent = '断开';
    document.getElementById('apiStatus').className = 'value red';
    if (el) { el.textContent = '❌ API 连接失败: ' + e.message; el.style.color = '#ff5f57'; }
  });
}

function calcStorage() {
  let total = 0;
  for (let key in localStorage) {
    if (localStorage.hasOwnProperty(key)) total += localStorage[key].length * 2;
  }
  const size = total < 1024 ? total + ' B' : (total/1024).toFixed(1) + ' KB';
  document.getElementById('lcSize').textContent = size;
}

function clearLogs() {
  if (!confirm('确定清空所有操作日志？')) return;
  localStorage.removeItem('aiwriter_logs');
  initLogs();
}

function resetData() {
  if (!confirm('确定重置所有本地数据？此操作不可撤销！')) return;
  if (!confirm('再次确认：重置本地存储数据？')) return;
  localStorage.clear();
  location.reload();
}

function exportLogs() {
  try {
    const logs = JSON.parse(localStorage.getItem('aiwriter_logs') || '[]');
    const blob = new Blob([JSON.stringify(logs, null, 2)], {type:'application/json'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'aiwriter-logs-' + new Date().toISOString().slice(0,10) + '.json';
    a.click();
    URL.revokeObjectURL(a.href);
  } catch(e) { alert('导出失败: ' + e.message); }
}

// Init
document.querySelectorAll('.console-sidebar-item[data-section]').forEach(el => {
  el.addEventListener('click', e => { e.preventDefault(); switchSection(el.dataset.section); });
});
document.getElementById('statusTime').textContent = new Date().toLocaleString();
setInterval(() => { document.getElementById('statusTime').textContent = new Date().toLocaleString(); }, 1000);
switchSection('overview');
  </script>
</body>
</html>`,
};
