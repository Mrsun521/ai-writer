...
const WORKER_URL = 'https://ai-writer.ai-api-svc.workers.dev';
...
function bindConsoleEvents() {
  ...
  document.getElementById('topupConfirm')?.addEventListener('click', async () => {
    ...
    try {
      // Try real API first
      const resp = await fetch(WORKER_URL + '/api/pay/create-order', {
        method: 'POST',
      ...
      if (resp.ok) {
        ...
        pollTimer = setInterval(async () => {
          try {
            const statusResp = await fetch(WORKER_URL + '/api/pay/order-status?order_id=' + currentOrderId);
            if (statusResp.ok) {
              ...
              if (statusData.status === 'completed') {
                ...
                payLinkArea.style.display = 'none';
                // Refresh balance
                const usageResp = await fetch(WORKER_URL + '/api/usage');
                if (usageResp.ok) {
